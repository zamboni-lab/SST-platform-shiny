...
server = function(input, output, session) {
  ...
  output$summary_plot = renderPlot({ plot_qc_summary(qc_values(), input) }, height = 600)
  ...
  output$score = renderUI({
    HTML(paste("<b>Score:</b>", get_run_score(qc_values(), input), "QC characteristics are within good ranges.", sep = " "))
  })
